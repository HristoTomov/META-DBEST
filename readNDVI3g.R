readNDVI3g <- function(data.file,data.size, data.type, data.endian, 
                       data.samples, data.lines, region.r, region.SPDF) {
        
        # Open and read the first found file
        f.in <- file(data.file,'rb')
        X <- readBin(f.in, integer(), n=data.size, size=data.type, 
                     endian=data.endian)
        close(f.in)
        ...
        r.template <- raster(nrows = data.samples, ncols = data.lines, 
                             ext = r.ext)
